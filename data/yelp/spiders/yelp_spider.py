...
class YelpSpider(Spider):
    ...
    start_urls = [
        'https://www.yelp.com/search?find_desc=restaurants&find_loc=miami&start=0']
    ...
    def parse(self, response):
        ...
        result_urls = ['https://www.yelp.com/search?find_desc=restaurants&find_loc=miami&start=' +
                       str(x) for x in range(0, total, 10)]
    ...
    def parse_result_page(self, response):
        ...
        restaurant_urls_ending = response.xpath(
            '//a[@class="biz-name js-analytics-click"]/@href').extract()[1:]

        # Manually concatenate all the urls
        restaurant_urls = ['https://www.yelp.com' +
                           url for url in restaurant_urls_ending]
    ...
    def parse_restaurant_page(self, response):
        ...
        text = response.xpath(
            '//span[@class = "review-count rating-qualifier"]/text()').extract_first()
        total = int(re.findall('\d+', text)[0])
        restaurant_review_urls = [
            truncated_url + '?start=' + str(x) for x in range(0, total, 20)]
    ...
    def parse_restaurant_reviews_page(self, response):
        ...
        restaurant = response.xpath(
            '//div[@class = "biz-page-header-left claim-status"]/div/h1/text()').extract_first().strip()
        address = response.xpath(
            '//div[@class="mapbox"]//address/text()').extract_first().strip()
        price = response.xpath(
            '//span[@class="business-attribute price-range"]/text()').extract_first()
        for review in reviews:
            text = review.xpath('.//p[@lang="en"]/text()').extract()
            rating = review.xpath(
                './/div[@class="biz-rating biz-rating-large clearfix"]/div/div/@title').extract_first()[0]
            date = review.xpath(
                './/span[@class="rating-qualifier"]/text()').extract_first().strip()

            if "vegan" in text[0].lower() and int(rating[0]) > 2:
